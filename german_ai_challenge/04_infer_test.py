...
fid_test2A = h5py.File(os.path.join(data_base_dir, round2_testA_filename), 'r')
...
# Choose Channel
if channel == 'full': 
    test_data = np.concatenate([s1_test2A, s2_test2A], axis=-1)
    ...
    test_data = s1_test2A[...,4:]

elif channel == 's1_ch5678+s2':
    test_data = np.concatenate([s1_test2A[...,4:], s2_test2A], axis=-1)
